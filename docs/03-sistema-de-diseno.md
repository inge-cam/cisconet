...
:root {
  ...
  --gold-600:  #B0834A;  /* hover de botón primario, texto de énfasis */
...
```css
/* Hero oscuro (Inicio, Agenda, Horarios) */
...
**Prohibido:** dorado sobre crema en texto corrido (no pasa AA). Sobre fondo claro el dorado solo se admite en piezas grandes o gruesas: títulos de 24 px o más, iconos, reglas.
...
- Los saltos de línea del hero son intencionales: `<br>` solo en escritorio (`hidden md:inline`), nunca en móvil.
...
| 3 | **Horarios** | `InfoBar` de 4 celdas: entre semana · dominicales · confesiones · despacho + enlace a `/horarios` |
...
**Los bloques 3 y 5 no se mueven.** El 3 debe verse **sin hacer scroll** en escritorio: por eso el hero mide 620 px y la `InfoBar` se superpone con `-72px`. En móvil, el bloque 3 queda inmediatamente bajo el pliegue, a un gesto de distancia.
...
Tailwind leyendo los tokens desde variables CSS, para tener **una sola fuente de verdad**:
...
Astro estático por defecto. Solo cinco piezas llevan JS, con JS nativo dentro de islas (`client:visible` o `client:idle`):

1. Menú móvil y encabezado que cambia con el scroll
2. Carrusel de celebraciones de la portada
3. Lightbox de galería
4. Validación y envío de formularios
